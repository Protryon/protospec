    ...
    fn decode_enum_container(
        ...
        for (name, child) in type_.items.iter() {
            ...
            match &*subtype {
                ...
                _ => {
                    let decoded = decoded.first().expect(
                        "enum discriminant was proper interior container, which is illegal",
                    );
                    self.instructions.push(Instruction::Construct(
                        target,
            ...
            if let Some(condition) = condition {
                ...
                self.instructions
                    .push(Instruction::ConditionalPredicate(condition, drained));
            } else {
                return vec![target];
            }
        }
        self.instructions.push(Instruction::Error(format!(
            "no enum conditions matched for {}",
            field.name
        )));
        vec![]
    }
