...
pub enum Instruction {
    ...
    ConditionalWrapStream(usize, Vec<Instruction>, Target, usize, Arc<Transform>, Vec<usize>), // condition, prelude, stream, new stream, transformer, arguments
...
impl Context {
    ...
    pub fn decode_field_top(&mut self, field: &Arc<Field>) -> usize {
        ...
        match &*field.type_.borrow() {
            ...
            _ => {
                ...
                self.instructions.push(Instruction::Construct(extra_value, Constructable::TaggedTuple { name: field.name.clone(), items: vec![value] }));
                return extra_value;
            },
        }
        value
    ...
    pub fn decode_field(&mut self, mut source: Target, field: &Arc<Field>) -> Option<usize> {
        ...
        for transform in field.transforms.borrow().iter().rev() {
            ...
            if let Some(condition) = condition {
                ...
                self.instructions.push(Instruction::ConditionalWrapStream(condition, drained, source, new_stream, transform.transform.clone(), args));
            } else {
                self.instructions.push(Instruction::WrapStream(source, new_stream, transform.transform.clone(), args));
            }
        ...
        let emitted = match &*field.type_.borrow() {
            Type::Container(c) => {
                let buf_target = if let Some(length) = &c.length {
                    ...
                    self.instructions.push(Instruction::Constrict(source, buf, len_register));
                    Target::Stream(buf)
                } else {
                    source
                };
        ...
        if let Some(field_condition) = field_condition {
            ...
            self.instructions.push(Instruction::Conditional(target, emitted.unwrap(), field_condition, drained));
            Some(target)
        } else {
            emitted
        }
    ...
    pub fn decode_type(&mut self, source: Target, field: &Arc<Field>) -> usize {
        ...
        match &*field.type_.borrow() {
            ...
            Type::Enum(e) => {
                self.instructions.push(Instruction::DecodeEnum(field.name.clone(), PrimitiveType::Scalar(e.rep.clone()), output, source));
                output
            },
            Type::Scalar(s) => {
                self.instructions.push(Instruction::DecodePrimitive(source, output, PrimitiveType::Scalar(*s)));
                output
            },
            Type::F32 => {
                self.instructions.push(Instruction::DecodePrimitive(source, output, PrimitiveType::F32));
                output
            },
            Type::F64 => {
                self.instructions.push(Instruction::DecodePrimitive(source, output, PrimitiveType::F64));
                output
            },
            Type::Bool => {
                self.instructions.push(Instruction::DecodePrimitive(source, output, PrimitiveType::Bool));
                output
            },
            Type::Foreign(f) => {
                self.instructions.push(Instruction::DecodeForeign(source, output, f.clone(), vec![]));
                output
            },
            Type::Ref(r) => {
                let mut args = vec![];
