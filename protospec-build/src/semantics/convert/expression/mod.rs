...
impl Scope {
    pub fn convert_expr(
        ...
        let expression = match expr {
            ...
            ArrayIndex(expr) => Expression::ArrayIndex(Self::convert_array_index_expression(
                self_,
                expr,
                expected_type,
            )?),
            EnumAccess(expr) => Expression::EnumAccess(Self::convert_enum_access_expression(
                self_,
                expr,
                expected_type,
            )?),
            Int(expr) => Expression::Int(Self::convert_int_expression(self_, expr, expected_type)?),
            Bool(expr) => {
                Expression::Bool(Self::convert_bool_expression(self_, expr, expected_type)?)
            }
            Ref(expr) => Self::convert_ref_expression(self_, expr, expected_type)?,
            Str(expr) => Self::convert_str_expression(self_, expr, expected_type)?,
            Ternary(expr) => Expression::Ternary(Self::convert_ternary_expression(
                self_,
                expr,
                expected_type,
            )?),
...
impl Expression {
    pub(crate) fn assign_to_or_coerce_to(
        mut self,
        expected_type: PartialType,
        span: Span,
    ) -> AsgResult<Self> {
